...
# ruta del archivo utilizado para generar el diccionario
RUTA_DICT = "diccionarios/supergigante.txt"
...
def obtener_diccionario():
    """
    Carga el diccionario desde el archivo y lo devuelve como un set.
    """
    ruta = os.path.join(os.path.dirname(__file__), RUTA_DICT)
    if not os.path.isfile(ruta):
        raise FileNotFoundError("El archivo no se pudo encontrar")
    
    with open(ruta, "r") as f:
        try:
            diccionario = set(f.read().splitlines())
        except:
            raise ValueError("Hubo un error al leer el archivo")
    return diccionario